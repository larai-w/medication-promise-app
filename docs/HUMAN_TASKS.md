...
最終更新: 2026-07-15（一世帯限定MVPの安全対応後）

Fable / Sonnet / Opus が実行できず、**人間のアカウントアクセスが必要なタスク**をまとめています。
優先度順に並んでいます。上から順にやればOKです。

> 最新の作業ログは `docs/SESSION_NOTE_2026-07-15_MVP.md` を参照。
> WebとAlexaのコード反映・自動テスト・非破壊スモークテストは完了済み。人間にしかできない最優先作業は **H-03とH-06の実機確認**。

---

## 🔥 いま人間が判断すべきこと（最優先）

### 動作確認: Alexa実機と本番Webを実際に触る（15分）
👉 **https://dhr30db6tf09e.cloudfront.net** をスマホ／PC で開く。
共有されたアクセスコードでログインし、記録のワンタップ・削除モーダル・月次 PDF・完了率%を確認する。続けてAlexaの記録とリマインダーを確認する（合格基準は下の H-06 表）。
...
**優先度: P0** | **2026-07-15に自動デプロイ・LaunchRequest確認済み**

特定薬名の固定文言除去、リマインダー設定化、夜9時の安全な応答、エラー処理を本番 `DrugAndOathFunction` に反映済み。人間の作業は不要。
...
HTTP/APIとAlexa Lambdaの自動スモークテストは完了。端末固有の表示・音声認識・権限は自動確認できないため、下表だけ人間が実機で確認する。
...
- カテゴリ: 健康とフィットネス
- プライバシーポリシー URL: 本番Webの `/privacy`（限定ベータ向け案は公開済み。一般申請前に内容確認）
...
| H-04 Lambda デプロイ | 完了 | 0分 | ✅ |
| H-05 Amplify デプロイ | 今すぐ | 20〜30分 | ⬜ |
| H-06 実機での動作確認 | 今すぐ | 10〜15分 | 🔄 |
| **PHASE 0 合計** | | **55〜75分** | |
| H-07 ブランチ保護 | 今週中 | 5分 | ⬜ |
